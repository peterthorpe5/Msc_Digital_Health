...
def alter_gb_file(infile):
    ...
    for line in f_in:
        if line.startswith("LOCUS"):
            ...
            space = " " * (len(contig) - len(contig_new))
            replacment_contig = ("%s%s") % (contig_new, space)
            # add the length in
            len_cont = len(length)
            replacment_contig = replacment_contig[:-len_cont]
            replacment_contig_w_len = replacment_contig + length
            while len(replacment_contig_w_len)> 30:
                replacment_contig = replacment_contig[:-1]
                replacment_contig_w_len = replacment_contig + length
                
            line = line.replace(contig, replacment_contig_w_len)
...
for filename in os.listdir("."):
    if filename.endswith(".gbf") or filename.endswith(".gbk") or filename.endswith(".gb"):
        for seq_record in SeqIO.parse("Chlamydia_trachomatis_plasmid_pUCH-1.gb", "genbank"):
