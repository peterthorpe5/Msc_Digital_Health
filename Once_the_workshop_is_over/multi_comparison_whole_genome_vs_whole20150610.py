...
from reportlab.lib.colors import red, grey, green, yellow, orange, green, brown, blue, lightblue, purple
...
MIN_GAP_JAGGY = 1000 # changed from None #Was 1000 # Sigils
MIN_GAP = 20000
...
for i, assembly_fasta in enumerate(assemblies_fasta):
    ...
    for contig in contigs:
        ...
        for feature in contig.features:
            ...
            gd_contig_features.add_feature(feature, sigil="ARROW",
        ...
        for hsp in blast_hsps:
            if hsp.hit_strand == -1:
                if hsp.bitscore >= 7000 and hsp.bitscore < 12000:
                    flip = True
                    color = colors.green
                if hsp.bitscore >= 12001:
                    flip = True
                    color = colors.red
                    #print "actually im here"
                if hsp.bitscore < 7000 and hsp.bitscore >2001:
                    flip = True
                    color = colors.blue
                if hsp.bitscore < 2000:
                    flip = True
                    color = colors.yellow
                    #print "im here"
...
print("Attemping page size %0.2fcm x %0.2fcm" % (30+5*len(assemblies_fasta), 100*max_len/500000))
page = (30*cm + 5*cm*len(assemblies_fasta), 100*cm*max_len/500000)
